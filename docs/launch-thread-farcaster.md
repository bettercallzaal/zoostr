...
> Shape: announcement → proof → mechanics → anthem → world opening.
> Replace `[STATS]`, `[TOKEN_ADDRESS]`, `[TOP_EARNERS]` with live `npm run receipt` output.
...
*(attach: zoostr.xyz OG card or the /receipt Farcaster frame)*
...
*(attach: zoostr.xyz/receipt frame)*
...
• fee pool allocated by leaderboard points — weekly
• claim at splits.org — your share waits, no deadline

the split is on-chain. anyone can verify it.

$ZOOSTR contract: [TOKEN_ADDRESS]
splits contract: [SPLITS_ADDRESS]
```
