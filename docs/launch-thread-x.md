# Zoostr — X (Twitter) Launch Thread

> Draft. Run `npm run receipt` before posting to get live empire stats.
> X allows longer posts — consolidate the 5 Farcaster casts into 3 posts here.
> Cross-post the same day as the Farcaster thread.
> Replace `[STATS]`, `[TOKEN_ADDRESS]`, `[TOP_EARNERS]` with live receipt numbers.

---

## Post 1 — Announcement + Proof (combined)

```
$ZOOSTR is live on Base.

34 people built the ZABAL empire on Boostr before any token existed.

The first split:
[TOP_EARNERS — paste from `npm run receipt`]
[e.g.:]
→ @[username]: 14.2% of the pool — $28/week
→ @[username]: 11.1% — $22/week
→ @[username]: 8.3% — $16/week

Every $ZOOSTR trade now pays them back. Weekly. On-chain. No claiming required.

zoostr.xyz
```

*(attach OG card or screenshot of zoostr.xyz/receipt)*

---

## Post 2 — Mechanics

```
how it works:

$ZOOSTR trades on Base (Clanker, 1% fee)
50% of every fee → a 0xSplits contract
splits.org distributes by leaderboard points — weekly
fees land in your wallet passively

the split is fully on-chain and public.

$ZOOSTR: [TOKEN_ADDRESS]
0xSplits: [SPLITS_ADDRESS]
```

---

## Post 3 — Anthem + CTA

```
34 people showed up before there was a token.

that's not a community. that's an empire.

$ZOOSTR is what an empire pays its builders with.

——

this is the first Sparkz launch. Sparkz builds the culture first, the token later — if at all.

want to launch like this? → sparkz.xyz

ZAO × Boostr × the empire. 🟡
```

---

## Notes

- X thread cadence: post all 3 at once and pin post #1.
- If cross-posting to a general tech/creator audience, lead with "back the work" not "$ZOOSTR".
- Tag @cashlessman (Aziz) + @bettercallzaal in post #1.
- For wide-channel (newsletters, creator forums): use the "mechanics" post as a standalone explainer.
- Hashtags (optional, low priority): #CreatorCoin #Sparkz #Base
- Avoid: "buy", "investment opportunity", "massive gains". Frame = contribution → compensation.

---

*Prepared by ZOL · 2026-07-17 · Placeholder stats — replace with live receipt before posting*
