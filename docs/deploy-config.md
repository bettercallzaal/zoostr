...
| 271 | `feat/how-it-works-about-pages` | add /how-it-works (6-step cycle, pool math, 50/25/25 split bar) + /about (ZABAL × Boostr origin story, timeline, creator bios, stats, culture-first principle) + sitemap + llm.txt (PR #249) |
...
| 274 | `feat/boostr-page-and-links` | add /boostr landing page (Boostr × Zoostr partnership, 5-step earn flow, API details) + homepage link in Boostr section + sitemap + llm.txt (PR #252) |
| 275 | `feat/deploy-config-pr-table-273` | deploy-config.md: update PR merge table 272→274 rows; "After all 272 PRs" → "After all 275 PRs" |

After all 275 PRs are merged to `main`:
